...
def ML_evaluate(model, X, y, h):
    ...
    def get_TS_cv(k=10):
        ...
        """
        return TimeSeriesSplit(n_splits=k, gap=0)
...
def get_data():
    """
    aggregate embedding by daily mean
    """
    df_news_price = pd.read_pickle(
        f"{HOME}/embedding/fasttext/WTI_Spot_n_RedditNews_2008-06-09_2016-07-01_fasttext.pkl"
    )
    df_grouped = pd.DataFrame()
    df_grouped["News_fasttext"] = df_news_price.groupby(
        "Date").News_fasttext.mean()
    df_grouped["Price"] = df_news_price.groupby("Date").Price.mean()
    return df_grouped
...
def main():
    ...
    for h in range(6):  # h: [0,6)
        X = np.array(df_Xy[f"{X_name}_t-{h}"].to_numpy().reshape(-1).tolist())
        y = df_Xy[f'{y_name}'].to_numpy().reshape(-1)
        result = ML_evaluate(LinearSVR(), X, y, h=h)
        result["descriptions"] = "avg fasttext SVR"
    ...
    for h in range(6):  # h: [0,6)
        ...
        result["descriptions"] = "avg fasttext Ridge"
        df_result = df_result.append(pd.DataFrame(result), ignore_index=True)
    ...
    for h in range(6):  # h: [0,6)
        ...
        result["descriptions"] = "avg fasttext SGDRegressor"
        df_result = df_result.append(pd.DataFrame(result), ignore_index=True)
