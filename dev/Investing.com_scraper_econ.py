    ...
    try:
        ...
        for p in range(1, 2000):
            for retry in range(5):
                try:
                    driver.get(
                        f"https://www.investing.com/news/economic-indicators/{p}")
                    ...
                    for each in df.to_dict(orient='records'):
                        try:
                            ...
                            if e.details["code"] == 11000:
                                ...
                                logging.exception(
                                    f"Investing.com econ scraper(p= {p}; retry={retry}) writing to MongoDB: {e}"
                                )
                    break
                except Exception as e:
                    logging.exception(f"Investing.com econ scraper(p= {p}; retry={retry}): {e}")
                    logging.exception(f"results: {results}")
                    time.sleep(5)
        driver.close()
        logging.info("Investing.com econ scraper finished job.")
    except Exception as e:
        logging.exception(f"Investing.com econ scraper: {e}")
        driver.close()
        time.sleep(5)
