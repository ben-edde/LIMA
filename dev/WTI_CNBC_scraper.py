...
def main():
    ...
    for t in range(5):
        ...
        try:
            ...
            driver = webdriver.Firefox(executable_path=f"{HOME}/dev/drivers/geckodriver",options=opts)
            all_results = []
            for i in range(1, 5):
                try:
                    label = f"CLC{i}"
                    driver.get(f"https://www.cnbc.com/quotes/{label}")
                    summary_label = driver.find_elements(By.CLASS_NAME,
                                                        "Summary-label")
                    summary_value = driver.find_elements(By.CLASS_NAME,
                                                        "Summary-value")
                    results = [(each[0].text, each[1].text)
                            for each in zip(summary_label, summary_value)]
                    df = pd.DataFrame(results)
                    df.columns = ["Type", "Data"]
                    closing_price = df[df.Type == "Prev Close"].Data.to_numpy()[0]
            ...
            try:
                ...
                df_p.index=df_p.index.map(lambda x: datetime.datetime.combine(x,datetime.time(22,0)))
                df_p=df_p.astype('double')
                client= InfluxDBClient.from_config_file(f"{HOME}/dev/DB/influxdb_config.ini")
                write_api=client.write_api(write_options=SYNCHRONOUS)
                write_api.write(bucket="dummy",record=df_p, data_frame_measurement_name='WTI')
                client.close()
            except Exception as e:
